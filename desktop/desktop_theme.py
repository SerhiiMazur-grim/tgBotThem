...
async def create_zip_archive(image_file, text_file, output_filename):
    # Відкриття zip архіву для запису
    with zipfile.ZipFile(output_filename, 'w') as zipf:
        # Додавання зображення до архіву з переіменуванням
        zipf.write(image_file, f'background.{image_file.split(".")[-1]}')

        # Додавання текстового файлу до архіву з переіменуванням
        zipf.write(text_file, f'colors.{text_file.split(".")[-1]}')


async def create_pc_theme(chat_id, image_path, background_color, primary_text_color,
                          secondary_text_color, background_alfa):

    background_color2 = await adjust_color_brightness(background_color)
    background_alfa = alfa_bg[background_alfa]
    ...
    user_dir = os.path.join('desktop', 'theme', str(chat_id))
    if not os.path.exists(user_dir):
        os.makedirs(user_dir)

    txt_file_path = os.path.join('desktop', 'theme', str(chat_id), file_name)
