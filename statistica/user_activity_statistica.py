...
async def user_activity_per_day(callback_query: CallbackQuery, session: AsyncSession):
    ...
    users = await session.scalars(select(User).where(and_(
        User.active==True,
    ...
    chats = await session.scalars(select(User).where(and_(
        User.active==True,
        User.chat_type!='private',
        User.last_active>=one_day
    )))
    ...
    prem_users_count = len([user for user in users if user.premium==True])
